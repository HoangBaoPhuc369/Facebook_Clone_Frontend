...
import { viewNegativePostShare } from "../../redux/features/postSlice";
import ModalCustom from "../Modal";

export default function PostShare({ user, post, postParentId }) {
  const [isOpenNegativePost, setIsOpenNegativePost] = useState(false);

  const dispatch = useDispatch();

  const handleShowNegativePost = (id) => {
    dispatch(viewNegativePostShare(id));
        ...
        footer={
          <>
            <button
              ...
              onClick={() => {
                handleShowNegativePost(postParentId);
              }}
            >
              Continue
            </button>
